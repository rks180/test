...
export async function search(req: Request, res: Response): Promise<void> {
  ...
  if (!username) {
    res.status(400).json({ error: 'Query param "username" is required' });
    return;
  }
  ...
  const nameFilter = { firstname: new RegExp(req.query.exact === 'true' ? `^${re}$` : re, 'i') };

  const users = await User.find(nameFilter, { firstname: 1, email: 1, dob: 1 }).lean();
  if (!users.length) {
    res.status(404).json({ error: `No user matches username "${username}"`, policies: [] });
    return;
  }
...
export default { search, aggregateByUser };
